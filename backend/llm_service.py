...
_PROVIDER_CONFIGS: Dict[str, ProviderConfig] = {
    ...
    # ── LLM7 turbo models (source: /models API, all tier=turbo, model_type=chat) ──
    ...
    "llm7-deepseek-v4-flash-0731": ProviderConfig(
        ...
        # $0.02 in / $0.04 out | 400K ctx | reasoning+tools | avail: 100%
    ),
    "llm7-codestral-latest": ProviderConfig(
        name="llm7", model_id="codestral-latest", max_tokens=8192,
        # $0.01 in / $0.02 out | 32K ctx  | tools | avail: ~99.7%
    ),
    "llm7-gemma4-31b": ProviderConfig(
        name="llm7", model_id="gemma4:31b", max_tokens=8192,
        # $0.03 in / $0.08 out | 262K ctx | tools | avail: ~98.3%
    ),
    "llm7-minimax-m2.7": ProviderConfig(
        name="llm7", model_id="minimax-m2.7", max_tokens=8192,
        # $0.03 in / $0.05 out | 180K ctx | reasoning+tools | stream:false | avail: ~96.2%
    ),
    "llm7-mistral-nemo": ProviderConfig(
        name="llm7", model_id="mistral-Nemo-Instruct-2407", max_tokens=8192,
        # $0.03 in / $0.03 out | 128K ctx | json_mode | avail: ~99.6%
    ),
}


def _resolve_provider_chain(primary: str) -> List[ProviderConfig]:
    """Return [primary_config, fallbacks...] for seamless resilience.

    - Groq primary  → chain other Groq models, then optionally NVIDIA.
    - LLM7 primary  → chain Groq models as fallback (LLM7 may have quota issues).
    - Other/NVIDIA  → no automatic fallbacks (unless FALLBACK_LLM_PROVIDER is set).
    """
    ...
    # If primary is an LLM7 model, fall back to Groq (LLM7 may hit quota / balance limits)
    if cfg and cfg.name == "llm7":
        llm7_groq_fallbacks = [
            _PROVIDER_CONFIGS["groq-120b"],
            _PROVIDER_CONFIGS["groq-20b"],
            _PROVIDER_CONFIGS["groq-qwen"],
        ...
        for fb in llm7_groq_fallbacks:
            if all(c.model_id != fb.model_id for c in chain):
                chain.append(fb)
        print(f"[llm_service] LLM7 provider chain: [{cfg.model_id}] + {len(llm7_groq_fallbacks)} Groq fallback(s)")
    ...
    for cfg in provider_chain:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                last_error = e
                err_str = str(e).lower()
                # 402 / 401 are permanent auth/billing errors — skip retries immediately
                is_permanent = any(
                    phrase in err_str
                    for phrase in ["402", "401", "insufficient_balance", "insufficient balance",
                                   "unauthorized", "invalid api key", "payment required"]
                )
                if is_permanent:
                    print(f"[llm_service] Permanent error on {cfg.name}/{cfg.model_id} ({e}) — skipping to next provider.")
                    break
                is_transient = any(
                    phrase in err_str
